...
const ALL_PROVIDERS: ProviderConfig[] = [
  ...
  },
  {
    ...
    label: 'LinkedIn',
    icon: <LinkedInIcon />,
    description: 'Inicia sesión con tu cuenta de LinkedIn',
    toggleKey: 'linkedin',
  },
];
...
const LinkedAccountsSection: React.FC = () => {
    ...
    try {
      const [userResult, togglesResult] = await Promise.all([
        ...
        supabase
          ...
          .select('oauth_google_link_enabled, oauth_azure_link_enabled, oauth_twitter_link_enabled, oauth_facebook_link_enabled, oauth_linkedin_link_enabled')
          .maybeSingle(),
      ]);
      ...
      if (togglesResult.data) {
        ...
        setLinkToggles({
          ...
          linkedin: d.oauth_linkedin_link_enabled ?? false,
        });
      }
    } catch {
  ...
  const handleLink = async (provider: 'google' | 'azure' | 'x' | 'facebook' | 'linkedin_oidc') => {
    setError('');
    setSuccess('');
    setActionLoading(provider);
    try {
      // El provider de Supabase no siempre coincide con el segmento de la ruta:
      // 'linkedin_oidc' se atiende en /auth/linkedin-callback. Para los demas el
      // string es el mismo, por eso hasta ahora bastaba interpolar el provider.
      const segmentoCallback: Record<string, string> = { linkedin_oidc: 'linkedin' };
      const redirectTo = `${window.location.origin}/auth/${segmentoCallback[provider] ?? provider}-callback`;
              ...
              <div className="flex items-center gap-2 ml-4 flex-shrink-0">
                {linked ? (
                  ...
                  provider.key !== 'email' && canLink && (
                    <button
                      onClick={() => handleLink(provider.key as 'google' | 'azure' | 'x' | 'facebook' | 'linkedin_oidc')}
