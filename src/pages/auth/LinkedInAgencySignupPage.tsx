...
const LinkedInAgencySignupPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!website.trim()) { setError('El sitio web o página de LinkedIn es obligatorio'); setIsLoading(false); return; }
    ...
    try {
      ...
      await supabase.from('user_auth_providers').upsert(
        { user_id: user.id, provider: 'linkedin_oidc', provider_user_id: user.id },
  ...
  return (
    <AgencySignupFormBody
      ...
      oauthProviderIcon={LinkedInIcon}
    />
  );
};

export default LinkedInAgencySignupPage;
