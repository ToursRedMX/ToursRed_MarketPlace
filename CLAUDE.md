...
- **Deuda de eslint: 2209 problemas** (2121 errores, 88 avisos) en 316 archivos — 1513 en `src/`, 695 en `supabase/functions/`. **El 73% es un solo patrón: 1615 `no-explicit-any`.** La base de `lint` se apretó el 11-sep-2026 (iba 267 por encima del conteo real, o sea 267 de holgura donde cabía una regresión). **El corte que bloquea es el TOTAL, no la fila**, así que un PR que añada un `react-hooks/set-state-in-effect` y a cambio borre dos `any` pasa en verde — bloquear por regla solo en las `react-hooks/*` está anotado en la entrada 23 de la bitácora, sin implementar. Los `any` no se tocan a bulto: cada uno pide saber qué devuelve la consulta.
