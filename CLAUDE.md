...
- **Deuda de eslint: 2199 problemas** (2111 errores, 88 avisos) en 316 archivos — ~1500 en `src/`, ~695 en `supabase/functions/`. **El 73% es un solo patrón: ~1615 `no-explicit-any`.** La base se apretó el 11-sep-2026 (iba 267 por encima del conteo real). **El corte que bloquea es el TOTAL, no la fila**, así que un PR que añada un `react-hooks/set-state-in-effect` y a cambio borre dos `any` pasa en verde — bloquear por regla solo en las `react-hooks/*` está anotado en la entrada 23, sin implementar. **Y cuidado con leer el número de una regla como deuda:** de los 118 `set-state-in-effect`, **58 son `useEffect(() => fetchX())`, el patrón de carga del repo, y no son bugs** — quedan 108 y la entrada 24 dice cuáles valen la pena y cuáles no.
- **Tipos del front: CERO, y la guardia exige cero.** `scripts/front-check/baseline.txt` quedó vacío el 11-sep-2026, así que cualquier error de tipos nuevo en `src/` bloquea —ya no es «sin errores nuevos sobre una línea base». Entró por el bullet 22 de la bitácora, que cuenta los nueve bugs reales que había detrás de los 72 últimos. **Esta línea decía «102 errores en 78 firmas» y la base tenía 72 en 57: la cifra del documento llevaba tiempo vieja.** También decía que los errores de `BookingSuccessPage` estaban rojos a propósito; el 11-sep-2026 ese archivo no tiene ninguno y tampoco los tenía en la base anterior, así que si esa pantalla se escribió contra columnas que no existen, **`tsc` ya no lo está señalando** y hace falta comprobarlo de otro modo antes de las UAT.
