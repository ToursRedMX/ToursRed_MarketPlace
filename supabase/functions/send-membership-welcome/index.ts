...
Deno.serve(async (req: Request) => {
  ...
  const guard = requireServiceRole(req, { recurso: "send-membership-welcome", cors: corsHeaders });
...
<body>
  <div class="container">
    ...
    return new Response(
      JSON.stringify({ error: "Error al enviar el email de bienvenida", details: mensajeDeError(error) }),
      {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      }
