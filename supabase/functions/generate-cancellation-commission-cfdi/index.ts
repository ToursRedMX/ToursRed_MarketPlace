...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: settings } = await supabase
      ...
      .select("pac_provider, pac_organization_id, cfdi_serie_booking, pac_issuer_rfc, pac_issuer_razon_social, pac_issuer_regimen_fiscal, pac_issuer_postal_code")
      .maybeSingle();
    ...
    EdgeRuntime.waitUntil(
      ...
      }).then((r: unknown) => vigilarResultado(r, "generate-cancellation-commission-cfdi -> send-cfdi-email")).catch((e: unknown) => registrarFallo("generate-cancellation-commission-cfdi -> send-cfdi-email", e))
    );
