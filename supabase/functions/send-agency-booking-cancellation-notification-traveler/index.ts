...
Deno.serve(async (req: Request) => {
  ...
  const guard = requireServiceRole(req, { recurso: "send-agency-booking-cancellation-notification-traveler", cors: corsHeaders });
  ...
  try {
    ...
    const originalDeposit = Number(cancellation.original_deposit_amount || 0);
