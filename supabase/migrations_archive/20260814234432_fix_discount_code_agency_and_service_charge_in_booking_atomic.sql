/*
# Fix discount code agency validation and service charge calculation in create_booking_atomic

## Problems Fixed

### Problem 1 — Agency discount codes not verified against tour's agency
When an agency creates a discount code restricted to its own tours
(discount_codes.agency_id points to that agency), create_booking_atomic
did not verify that restriction. It only checked is_active, expires_at,
and max_uses/current_uses. This allowed using an agency's discount code
on a tour belonging to a different agency.

Fix: Added `AND (dc.agency_id IS NULL OR dc.agency_id = v_agency_id)`
to the discount code SELECT. If the code has a non-null agency_id that
doesn't match the tour's agency, the code is treated as invalid
silently (no discount applied, no error blocking the booking).

### Problem 2 — Service charge calculated on list price instead of discounted price
The service charge that the traveler pays was calculated on the full
list price before the discount was resolved, then the discount was
subtracted from the combined total. This produced a slightly inflated
service charge.

Fix: Reordered the calculation so the discount code is resolved BEFORE
the service charge is computed. The service charge is now calculated
on `v_base_tour_price_discounted = GREATEST(0, v_base_tour_price - v_discount_amount)`.

### Problem 2b — Discount base narrowed to tour price only (business decision)
The discount amount is now calculated solely on `v_base_tour_price`
(not `v_base_tour_price + v_extras_subtotal`). Optional services/extras
are not affected by the discount in any part of the calculation.

### Problem 3 — Column name mismatch (bonus fix)
The original discount code query referenced `dc.expires_at` and
`dc.current_uses` which do not exist in the `discount_codes` table.
The actual columns are `valid_until` and `times_used`. The exception
handler silently swallowed the error, making ALL discount codes
non-functional. This migration fixes the column names so discount
codes actually work.

## Changes
1. Moved discount code validation block (was step 7) to before the
   service charge calculation (was step 3).
2. Added agency_id check to the discount code SELECT.
3. Fixed column names: expires_at → valid_until, current_uses → times_used.
4. Changed discount base from (v_base_tour_price + v_extras_subtotal)
   to v_base_tour_price only.
5. Introduced v_base_tour_price_discounted = GREATEST(0, v_base_tour_price - v_discount_amount).
6. v_base_service_charge now uses v_base_tour_price_discounted.
7. v_subtotal uses v_base_tour_price_discounted instead of v_base_tour_price.
8. v_grand_total no longer subtracts v_discount_amount (already reflected
   in the discounted base).
9. v_commission_amount still uses v_base_tour_price (list price, unchanged).
10. Points and wallet validation caps use v_base_tour_price_discounted.

## Scenario verification
Tour $5,000, 10% discount ($500), 5% service charge, 30% deposit:
- v_discount_amount = 5000 * 10% = $500
- v_base_tour_price_discounted = $4,500
- v_base_service_charge = 4500 * 5% = $225
- v_subtotal = 4500 + 0 + 0 + 0 + 225 + 0 = $4,725
- v_grand_total = $4,725
- v_deposit_amount = 4725 * 30% = $1,417.50
- v_commission_amount = 5000 * rate (on list price, unchanged)

## No new tables or columns
This migration only replaces the create_booking_atomic function.
*/
...
BEGIN
  ...
  IF COALESCE((p_booking_data->>'points_used')::integer, 0) > 0 THEN
    ...
    v_points_discount := LEAST(v_points_discount::numeric,
      GREATEST(0, v_base_tour_price_discounted + v_extras_subtotal + v_extras_sc + v_insurance_cost + v_deposit_sc + v_membership_cost));
  END IF;
  ...
  IF COALESCE((p_booking_data->>'toursred_cash_used')::numeric, 0) > 0 THEN
    ...
    v_wallet_discount := LEAST(
      ...
      v_wallet_balance,
      GREATEST(0, v_base_tour_price_discounted + v_extras_subtotal + v_extras_sc + v_insurance_cost + v_deposit_sc + v_membership_cost - v_points_discount)
    );
  END IF;
