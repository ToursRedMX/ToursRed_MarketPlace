/*
# Fix process_expired_slot_reschedules: move anon_key literal to Vault

## Problem
The function has the project's anon_key JWT hardcoded as a text literal in
the DECLARE block. While the two edge functions it calls have verify_jwt=false
(so any key works), having a valid JWT hardcoded in source is inconsistent
with the other cron-related functions that already read credentials from
Supabase Vault, and is less secure.

## Fix
Replace the v_anon_key literal with v_service_key read from Vault:
  v_service_key := (SELECT decrypted_secret FROM vault.decrypted_secrets WHERE name = 'service_role_key')

Update the 2 net.http_post calls that used 'Bearer ' || v_anon_key to use
'Bearer ' || v_service_key instead.

Everything else in the function is preserved exactly: same 2-step loop, same
refund logic, same bookings/tour_slots/notifications updates, same return
format, same SECURITY DEFINER, same search_path = public, same timeouts.
