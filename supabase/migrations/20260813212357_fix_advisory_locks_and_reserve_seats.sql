/*
# Add advisory_xact_lock to no-slot capacity validation + fix reserve_seats

## Part 2: Advisory lock for tours without slots

### Problem
Both `create_booking_atomic` and `activate_draft_booking` have two branches
for capacity validation: one for tours with slot_id (uses SELECT ... FOR UPDATE
on the slot row) and one for tours without slot_id (uses plain SELECT without
any lock). Two concurrent requests for the same slot-less tour could both
pass capacity validation before either commits, resulting in real oversells.

### Fix
In BOTH functions, in the branch that runs when slot_id IS NULL, add:
  PERFORM pg_advisory_xact_lock(hashtext(tour_id::text));
as the first statement before any capacity query. This serializes concurrent
calls for the same tour. The lock is released automatically when the
transaction ends.

## Part 3a: Fix reserve_seats ON CONFLICT DO UPDATE guard

### Problem
The `reserve_seats` function uses INSERT ... ON CONFLICT DO UPDATE to upsert
seat status rows. The DO UPDATE clause has no WHERE condition — it
unconditionally overwrites the existing row's status and booking_id. When two
concurrent requests compete for the same seat (both seeing it as available
because the FOR UPDATE only locks non-disponible rows), the second INSERT's
DO UPDATE overwrites the first one's booking_id without checking ownership.
Since the function is SECURITY DEFINER, RLS does not protect it.

### Fix
1. Add a WHERE clause to the ON CONFLICT DO UPDATE that only allows the
   update if the existing row's booking_id matches p_booking_id (same booking
   re-selecting its own seats) OR the existing row's status is 'disponible'
   (the seat was released and is truly free).
2. After each INSERT ... ON CONFLICT statement, check ROW_COUNT. If 0 rows
   were affected (the WHERE condition didn't match), add the seat to a
   conflict list.
3. If any seats are in the conflict list, return success: false with the
   conflicting_seats array. Otherwise return success: true.
...
-- ════════════════════════════════════════════════════════════════════════════
-- Part 2a: create_booking_atomic — add advisory lock in no-slot branch
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  ELSE
    -- Serialize concurrent bookings for the same slot-less tour
    PERFORM pg_advisory_xact_lock(hashtext(v_tour_id::text));
...
-- ════════════════════════════════════════════════════════════════════════════
-- Part 2b: activate_draft_booking — add advisory lock in no-slot branch
-- ════════════════════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════════════════════
-- Part 3a: Fix reserve_seats — guard ON CONFLICT DO UPDATE
-- ════════════════════════════════════════════════════════════════════════════
