/*
# Validate pickup & language service prices against real tour data

Services in p_optional_services without tour_optional_service_id
(pickup and language) were trusting unit_price from the client JSON.
Now the server recalculates from tour.pickup_zones and tour.tour_languages.
*/
...
BEGIN
  ...
  -- pickup_zones: [{"name":"Zona Hotelera","extra_cost":50,"cost_type":"por_persona"}, ...]
  ...
  -- tour_languages: [{"language":"Ingles","extra_cost":30,"cost_type":"por_persona"}, ...]
  v_language_cost := 0;
  ...
  IF p_optional_services IS NOT NULL AND jsonb_array_length(p_optional_services) > 0 THEN
    FOR v_service IN SELECT jsonb_array_elements(p_optional_services) LOOP
      ...
      IF v_svc_id IS NOT NULL AND v_svc_qty > 0 THEN
        -- Standard optional service: read real price from tour_optional_services
      ...
      ELSIF v_svc_kind = 'pickup' THEN
        -- Pickup: use pre-computed real cost from tour.pickup_zones
        v_svc_unit := v_pickup_zone_cost;
        v_svc_subtotal := v_pickup_zone_cost;
        v_svc_qty := 1;
      ELSIF v_svc_kind = 'language' THEN
        -- Language: use pre-computed real cost from tour.tour_languages
        v_svc_unit := v_language_cost;
        v_svc_subtotal := v_language_cost;
        v_svc_qty := 1;
      ELSE
        -- Unknown service kind without ID: zero cost
        v_svc_unit := 0;
        v_svc_subtotal := 0;
      END IF;
