/*
# Fix missing cast to approval_status enum in create_booking_atomic

## Purpose
The `create_booking_atomic` function inserts into the `bookings` table without
casting the `approval_status` value to the custom ENUM type `approval_status`.
While `status` and `payment_status` are plain `text` columns (no cast needed),
`approval_status` is a USER-DEFINED enum type and requires an explicit cast.

Without the cast, PostgreSQL implicitly casts the text value, which works in
most cases but can fail in certain contexts (e.g., when the value comes from
a jsonb->> extraction in a COALESCE). Adding the explicit cast makes it robust.

## Changes
1. Recreate `create_booking_atomic` with `COALESCE(p_booking_data->>'approval_status', 'approved')::approval_status`
   instead of the uncasted `COALESCE(p_booking_data->>'approval_status', 'approved')`.
   No other changes to the function body.
*/

-- ============================================================
-- Recreate create_booking_atomic with explicit approval_status cast
-- ============================================================
