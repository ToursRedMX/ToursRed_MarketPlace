/*
# Fix request_supplement_with_lock: remove JavaScript Number() call

## Problem
The function used `Number(v_supplement.price)` to calculate the subtotal.
`Number()` is a JavaScript function, not a PL/pgSQL function. Since
`v_supplement.price` is already `numeric`, the correct syntax is a direct
multiplication: `v_supplement.price * p_quantity`.

The `CREATE FUNCTION` succeeded because PL/pgSQL doesn't validate expression
types at creation time for dynamic SQL, but every runtime call would fail with
"function number(numeric) does not exist".

## Fix
Replace `Number(v_supplement.price) * p_quantity` with
`v_supplement.price * p_quantity`.
*/
