-- create_booking_atomic calcula el descuento de tour_promotions y lo guarda
-- en bookings.promotion_id / promo_discount_amount, pero nunca incrementaba
-- tour_promotions.times_used. No existe (ni existio) ninguna otra funcion que
-- lo hiciera: el contador ha estado muerto desde que se creo el sistema de
-- promociones grupales — por eso una promocion con reservas reales sigue
-- mostrando "0 usos".
--
-- Se agrega el incremento justo despues de calcular v_promo_discount. Para
-- nxprecio se incrementa por el numero de grupos que de verdad se cobraron al
-- precio especial (v_promo_grupos_con, ya respeta max_uses); para el resto
-- (2x1, 3x2, grupo_precio_fijo) es un uso por reserva.
...
-- ---- 1.6 INCREMENT tour_promotions.times_used WHEN THE PROMO ACTUALLY APPLIED ----
-- Nunca se incrementaba: el contador quedaba en 0 para siempre sin importar
-- cuantas reservas usaran la promocion. Para nxprecio se cuenta por grupo
-- (v_promo_grupos_con ya respeta max_uses); para el resto, un uso por reserva.
IF v_promo_id IS NOT NULL AND v_promo_discount > 0 THEN
UPDATE public.tour_promotions
SET times_used = times_used + (CASE WHEN v_promo_type = 'nxprecio' THEN GREATEST(v_promo_grupos_con, 1) ELSE 1 END)
WHERE id = v_promo_id;
END IF;
