/*
# Create request_supplement_with_lock function

## Problem
The Edge Function `request-supplement` checks supplement capacity via an RPC
call, then inserts a booking_supplements row in a separate request. Each
Supabase RPC call is its own transaction, so an advisory lock acquired in one
call would be released before the INSERT in the next call. Two concurrent
requests could both pass the capacity check before either inserts, resulting
in oversold supplements.

## Fix
Create a new SECURITY DEFINER function `request_supplement_with_lock` that
performs all operations in a single transaction:
1. Acquires pg_advisory_xact_lock(hashtext(tour_supplement_id)) — serialized
   per supplement, released at transaction end.
2. Validates the booking belongs to the user and is not cancelled.
3. Validates the supplement is active and belongs to the booking's tour.
4. Checks available capacity (max_capacity - active bookings).
5. Checks for duplicate active request.
6. Calculates service charge, commission, and expiry.
7. Inserts the booking_supplements row.
8. Returns the created record as JSON with success: true, or an error with
   success: false.
...
- Uses auth.uid() to verify the booking belongs to the calling user.
*/
...
BEGIN
  ...
  v_subtotal := Number(v_supplement.price) * p_quantity;
