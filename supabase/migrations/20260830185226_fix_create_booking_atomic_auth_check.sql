...
--
--   version: 20260830185226
--   name:    fix_create_booking_atomic_auth_check
--
...
-- ---- SECURITY: el creador de la reserva debe ser el usuario autenticado ----
-- Antes cualquiera (incluso sin sesion, via anon key) podia crear una reserva
-- atribuida a otro user_id llamando el RPC directo por PostgREST.
IF auth.uid() IS NULL OR auth.uid() IS DISTINCT FROM v_user_id THEN
RETURN jsonb_build_object('success', false, 'error', 'No autorizado: el user_id no coincide con la sesion autenticada');
END IF;
...
CREATE OR REPLACE FUNCTION public.create_booking_atomic_with_preventa(p_booking_data jsonb, p_travelers jsonb DEFAULT '[]'::jsonb, p_optional_services jsonb DEFAULT '[]'::jsonb, p_session_id text DEFAULT NULL::text, p_seat_numbers integer[] DEFAULT NULL::integer[])
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
DECLARE
  v_user_id uuid := (p_booking_data->>'user_id')::uuid;
  v_tour_id uuid := (p_booking_data->>'tour_id')::uuid;
  v_membership_purchased boolean := COALESCE((p_booking_data->>'membership_purchased')::boolean, false);
  v_preventa_result jsonb;
  v_is_preventa boolean;
  v_eligible boolean;
  v_preventa_error text;
BEGIN
  -- SECURITY: el creador de la reserva debe ser el usuario autenticado.
  IF auth.uid() IS NULL OR auth.uid() IS DISTINCT FROM v_user_id THEN
    RETURN jsonb_build_object('success', false, 'error', 'No autorizado: el user_id no coincide con la sesion autenticada');
  ...
  -- Check preventa eligibility before proceeding
  v_preventa_result := public.check_preventa_eligibility(v_tour_id, v_user_id, v_membership_purchased);
  v_is_preventa := (v_preventa_result->>'is_preventa')::boolean;
  v_eligible := (v_preventa_result->>'eligible')::boolean;

  IF v_is_preventa AND NOT v_eligible THEN
    v_preventa_error := v_preventa_result->>'error';
    RETURN jsonb_build_object(
      'success', false,
      'error', COALESCE(v_preventa_error, 'Este tour esta en preventa exclusiva para socios ToursRed Plus')
    );
  END IF;

  IF v_is_preventa AND v_eligible THEN
    p_booking_data := jsonb_set(p_booking_data, '{es_reserva_preventa}', 'true');
  ELSE
    p_booking_data := jsonb_set(p_booking_data, '{es_reserva_preventa}', 'false');
  END IF;

  RETURN public.create_booking_atomic(p_booking_data, p_travelers, p_optional_services, p_session_id, p_seat_numbers);
END;
$function$;
