...
--
--   version: 20260831054728
--   name:    fix_request_supplement_with_lock_verify_caller
--
...
-- BUG: aceptaba p_user_id sin verificar que coincidiera con auth.uid() (o
-- que la llamada viniera de service_role). Expuesta a anon. El Edge
-- Function que la llama SI verifica la identidad real del usuario antes de
-- pasar p_user_id, pero la funcion de base de datos por si sola no lo
-- garantiza -- cualquiera sin sesion que conociera un booking_id y su
-- user_id real podia crear solicitudes de suplemento a nombre de otro
-- usuario. Se agrega el chequeo dentro de la funcion como defensa en
-- profundidad, permitiendo NULL (llamadas de servicio) pero bloqueando un
-- auth.uid() que no coincida.
CREATE OR REPLACE FUNCTION public.request_supplement_with_lock(p_user_id uuid, p_booking_id uuid, p_tour_supplement_id uuid, p_quantity integer)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
booking_id, tour_supplement_id, quantity, unit_price, service_charge,
supplement_commission, total_paid, status, requested_at, expires_at
)
VALUES (
p_booking_id, p_tour_supplement_id, p_quantity, v_supplement.price, v_service_charge,
v_supplement_commission, 0, v_initial_status, now(), v_expires_at
)
