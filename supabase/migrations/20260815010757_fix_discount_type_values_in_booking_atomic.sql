/*
# Fix discount_type comparison in create_booking_atomic

## Problem
The discount code application block inside create_booking_atomic compared
`v_discount_rec.discount_type` against the generic values 'percentage' and
'fixed'. However, the `discount_codes` table has a CHECK constraint that only
allows compound values like 'tour_percentage', 'tour_fixed',
'agency_tour_percentage', 'agency_tour_fixed', 'membership_percentage', etc.

Because 'percentage' and 'fixed' never match any real discount_type value,
the IF/ELSIF block never entered either branch, and v_discount_amount always
stayed at 0 -- meaning ALL discount codes were silently ignored, even with
correct column names and agency validation.

## Fix
Changed the comparison to recognize the 4 tour-related discount_type values:
- 'tour_percentage' and 'agency_tour_percentage' → percentage calculation
- 'tour_fixed' and 'agency_tour_fixed' → fixed amount

## No new tables or columns
Only replaces the create_booking_atomic function body.
*/
