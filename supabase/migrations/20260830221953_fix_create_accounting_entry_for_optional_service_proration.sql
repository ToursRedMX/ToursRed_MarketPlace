...
--
--   version: 20260830221953
--   name:    fix_create_accounting_entry_for_optional_service_proration
--
...
-- get_wallet_portion_for_booking_component.
IF v_bos.payment_method = 'toursred_cash' THEN
v_wallet_used := v_total_amount;
ELSIF v_bos.payment_method IS NULL THEN
v_wallet_used := public.get_wallet_portion_for_booking_component(p_bos_id::uuid, v_total_amount); -- placeholder, corregido abajo
ELSE
v_wallet_used := 0;
END IF;

-- Nota: get_wallet_portion_for_booking_component espera un booking_id, no un
-- bos_id -- se corrige la llamada usando el booking real:
IF v_bos.payment_method IS NULL THEN
