/*
# Server-side price recalculation in create_booking_atomic

The browser now only sends what the user *wants* (tour, travelers, services, seats,
payment provider, discount code, points to use, etc.). The server recalculates
every monetary value from authoritative database records.

Reproduces the cascading membership exemption logic from BookingFlowStep4.tsx
(lines 166-224): baseServiceCharge -> exemptionUsed (from monthly pool) ->
depositServiceCharge -> extrasServiceChargeBase -> remaining exemption ->
extrasServiceCharge -> subtotalBeforeDiscount -> grandTotal -> depositAmount.
*/
...
DECLARE
  ...
  v_agency_id         uuid    := (p_booking_data->>'agency_id')::uuid;
  v_slot_id           uuid    := NULL;
...
BEGIN
  ...
  -- ---- VALIDATE TOUR EXISTS & LOAD REAL DATA ----
  SELECT
    t.id,
    t.agency_id,
    t.price,
  ...
  IF p_optional_services IS NOT NULL AND jsonb_array_length(p_optional_services) > 0 THEN
    FOR v_service IN SELECT jsonb_array_elements(p_optional_services) LOOP
      ...
      IF v_svc_id IS NOT NULL AND v_svc_qty > 0 THEN
        ...
        END IF;
      ELSE
        -- Pickup/language services may not have a tour_optional_service_id;
        -- trust the client subtotal only for those (they come from tour-defined zones)
        v_svc_unit := COALESCE((v_service->>'unit_price')::numeric, 0);
      END IF;
  ...
  -- ---- 4. MEMBERSHIP EXEMPTION CASCADE (mirrors BookingFlowStep4 lines 166-203) ----
  v_has_membership := public.has_active_membership(v_user_id);

  IF v_membership_purchased THEN
    -- Buying membership now: full waiver, no pool consumption
    v_deposit_sc := 0;
    v_extras_sc := 0;
  ELSIF v_has_membership THEN
    -- Existing membership: consume from monthly pool atomically (deposit first, then extras)
    v_exemption_result := public.apply_membership_service_fee_exemption(v_user_id, v_base_service_charge);
    v_exemption_applied := COALESCE((v_exemption_result->>'exemption_applied')::numeric, 0);
    v_deposit_sc := COALESCE((v_exemption_result->>'net_service_charge')::numeric, v_base_service_charge - v_exemption_applied);

    v_exemption_result := public.apply_membership_service_fee_exemption(v_user_id, v_extras_sc_base);
    v_exemption_applied := COALESCE((v_exemption_result->>'exemption_applied')::numeric, 0);
    v_extras_sc := COALESCE((v_exemption_result->>'net_service_charge')::numeric, v_extras_sc_base - v_exemption_applied);
  ELSE
    -- No membership: pay full service charge
  ...
  IF COALESCE((p_booking_data->>'travel_insurance_included')::boolean, false) THEN
    ...
    -- Calculate real tour duration in days
    ...
    -- For receptivo, always 1 day
    IF v_tour.tour_type = 'receptivo' THEN
      v_tour_duration_days := 1;
    END IF;

    -- Validate client-sent insurance_days
  ...
  IF COALESCE((p_booking_data->>'points_used')::integer, 0) > 0 THEN
    ...
    -- Cap to remaining subtotal after discount
  ...
  -- ---- 9. ASSEMBLE TOTALS (mirrors BookingFlowStep4 lines 212-224) ----
  ...
  -- ---- INSERT OPTIONAL SERVICES (with recalculated values) ----
  IF p_optional_services IS NOT NULL AND jsonb_array_length(p_optional_services) > 0 THEN
    FOR v_service IN SELECT jsonb_array_elements(p_optional_services) LOOP
      v_svc_id := NULLIF(v_service->>'tour_optional_service_id', '')::uuid;
      v_svc_qty := COALESCE((v_service->>'quantity')::integer, 0);
