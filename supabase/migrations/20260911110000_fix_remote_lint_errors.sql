-- Arreglos de los hallazgos del linter de Supabase.
--
-- ESTE ARCHIVO ES UNA RECONSTRUCCION, Y CONVIENE SABERLO
--
-- El SQL de esta version se aplico a la base SIN pasar por un commit: el
-- 11-sep-2026 `migration-drift.yml` la reporto como "aplicada sin archivo", que
-- es exactamente lo que ese check vigila y la causa raiz de las 151 huerfanas
-- que se limpiaron el 02-sep-2026.
--
-- El texto de abajo se recupero de `supabase_migrations.schema_migrations`, o
-- sea del arreglo `statements[]` que guardo la base. Eso significa:
--
--   - Es una EXPORTACION FUNCIONAL, no el texto original. El formato esta
--     normalizado y los comentarios sueltos entre sentencias pueden haberse
--     perdido.
--   - Sirve para reproducir el esquema y para revisar QUE se cambio. No es el
--     registro literal de lo que alguien escribio.
--
-- UNA CORRECCION QUE HUBO QUE HACER A MANO
--
-- El exportador corta por `;`, y la ultima funcion
-- (`auto_accept_expired_slot_reschedules`) tiene `;` dentro de su cuerpo. Al
-- exportarla quedaba partida en mitad del bloque DECLARE —
-- `v_request record` y luego `;` en su propia linea— lo que la dejaba
-- SINTACTICAMENTE ROTA. Se recompuso juntando esos fragmentos.
--
-- NO HACE FALTA APLICARLA: ya esta aplicada. Este archivo existe para que el
-- repo y el ledger vuelvan a contar la misma historia.
...
-- OJO: esta funcion venia PARTIDA en la exportacion. El exportador corta por
-- `;` y esta los tiene dentro del cuerpo, asi que su DECLARE quedaba troceado
-- en fragmentos separados por `;` en lineas sueltas. Aqui esta recompuesta.
