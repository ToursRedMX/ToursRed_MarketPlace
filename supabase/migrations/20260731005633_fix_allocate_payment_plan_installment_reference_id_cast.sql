/*
# Fix: Remove ::text cast on reference_id in allocate_payment_plan_installment

## Problem
The function casts `v_tx_id::text` for the `reference_id` column in
`toursred_points_transactions`, but that column is type `uuid`, not `text`.
PostgreSQL rejects: "column 'reference_id' is of type uuid but expression is of type text".

This only triggers when the user has an active membership (v_points_earned > 0),
which is why it wasn't caught in code review — it requires a real points-award scenario.

## Fix
Use `v_tx_id` directly (already declared as `uuid`), matching the original TypeScript
code which passed `reference_id: txRecord.id` without conversion.
*/

-- Use CREATE OR REPLACE since only the function body changes, not the signature
