/*
# Fix: Remove pending_balance from allocate_payment_plan_installment UPDATE

## Problem
The `allocate_payment_plan_installment` function tries to SET `pending_balance` in the
UPDATE on `booking_payment_plans`, but `pending_balance` is a GENERATED ALWAYS AS
(total_plan_amount - total_amount_paid) STORED column — PostgreSQL rejects manual
assignment with: "column 'pending_balance' can only be updated to DEFAULT".

This breaks ALL payment plan installment payments (Stripe, PayPal, MercadoPago,
wallet, points, and Conekta), not just Conekta, because the refactor routed all
providers through this shared function.

## Fix
Drop and recreate the function with `pending_balance` removed from the UPDATE.
The column auto-recalculates when `total_amount_paid` changes, so no manual write needed.
*/
...
BEGIN
  ...
  -- Update plan totals — pending_balance is GENERATED ALWAYS AS, do NOT write it manually
